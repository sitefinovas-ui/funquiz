...
export const getAllMessagesData = async () => {
  try {
    const [rows] = await db.query("SELECT * FROM funquiz_messages ORDER BY created_at DESC");
    return rows;
  } catch (error) {
...
export const getMessageById = async (message_id) => {
  try {
    const [rows] = await db.query("SELECT * FROM funquiz_messages WHERE message_id = ?", [message_id]);
...
export const createMessage = async (data) => {
  try {
    const { user_id = null, name, email, subject, content, priority = "normal", status = "unread", assigned_to = null } = data;

    const [result] = await db.query(
      `INSERT INTO funquiz_messages (user_id, name, email, subject, content, priority, status, assigned_to) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [user_id, name, email, subject, content, priority, status, assigned_to]
    );
...
export const updateMessage = async (message_id, data) => {
  try {
    ...
    const { subject, content, priority, status, assigned_to } = data;
    const fields = [];
    const values = [];
...
export const deleteMessage = async (message_id) => {
  try {
    ...
    const [result] = await db.query("DELETE FROM funquiz_messages WHERE message_id = ?", [message_id]);
    if (result.affectedRows === 0) throw new Error("Message non trouvé.");
