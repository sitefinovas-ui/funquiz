import { getAllFaqData, getFaqById, createFaq, updateFaq, deleteFaq } from '../models/faqModel.js';
...
export const addFaq = async (req, res) => {
    try {
        ...
        if (!question || !answer) {
            return res.status(400).json({ error: 'Les champs question et answer sont requis.' });
        }
...
export const editFaq = async (req, res) => {
    try {
        ...
        if (!question && !answer && is_active === undefined) {
            return res.status(400).json({ error: 'Au moins un champ (question, answer, is_active) doit être fourni pour la mise à jour.' });
        }
