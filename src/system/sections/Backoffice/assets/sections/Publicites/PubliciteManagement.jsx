...
import publiciteServices from '../../../../../configurations/Services/publiciteServices.js';
...
export default function PubliciteManagement() {
  ...
  const fetchAll = async () => {
    setLoading(true);
    try {
      setItems(await publiciteServices.list());
    } catch {
      setStatus({ type: 'error', message: 'Impossible de charger les publicités.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await publiciteServices.update(editing.id, form);
        setStatus({ type: 'success', message: 'Publicité mise à jour.' });
      } else {
        await publiciteServices.create(form);
        setStatus({ type: 'success', message: 'Publicité créée.' });
      }
      setShowModal(false);
      fetchAll();
    } catch {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setStatus({ type: 'success', message: 'Publicité supprimée.' });
      fetchAll();
    } catch {
  ...
  const toggleStatut = async (item) => {
    ...
    try {
      await publiciteServices.update(item.id, { ...item, statut: next });
      fetchAll();
    } catch {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">Gérez les publicités et leurs liens d'action</p>
        </div>
        <button
          onClick={openAdd}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              ...
              <div className="p-5">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <button onClick={() => toggleStatut(item)} className="ml-3 flex-shrink-0 text-2xl">
                    {item.statut === 'actif'
                      ? <FaToggleOn className="text-green-500" title="Actif — cliquer pour désactiver" />
                      : <FaToggleOff className="text-gray-300" title="Inactif — cliquer pour activer" />}
                  </button>
                </div>
          ...
          {items.length === 0 && (
            <div className="col-span-2 py-16 text-center text-gray-400">Aucune publicité pour l'instant.</div>
          )}
        </div>
      )}
